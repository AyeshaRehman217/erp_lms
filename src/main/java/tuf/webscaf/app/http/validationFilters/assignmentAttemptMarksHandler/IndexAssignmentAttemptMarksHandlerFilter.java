...
public class IndexAssignmentAttemptMarksHandlerFilter implements HandlerFilterFunction<ServerResponse, ServerResponse> {
    @Override
    public Mono<ServerResponse> filter(ServerRequest request, HandlerFunction<ServerResponse> next) {
        ...
        // Optional Query Parameter of Student
        String studentUUID = request.queryParam("studentUUID").map(String::toString).orElse("").trim();

        // Optional Query Parameter of Assignment Attempt
        String assignmentAttemptUUID = request.queryParam("assignmentAttemptUUID").map(String::toString).orElse("").trim();
        ...
        return request.formData()
                .flatMap(value -> {
                    ...
                    if (!assignmentAttemptUUID.isEmpty()) {
                        if (!assignmentAttemptUUID.matches("^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$")) {
                            messages.add(
                                    new AppResponseMessage(
                                            AppResponse.Response.ERROR,
